...
if __name__ == "__main__":
    np.random.seed(1)  # 设置随机种子,train:3407;test:1

    rou_dir = 'four/test_603'
    # rou_dir = 'single/test_603'
    mkdir('../sumo_sim_env/' + rou_dir)

    cav_penetration = 0.2
    # space_imbalance_ratio = 0.3 if 'single' in rou_dir else 0.1
    space_imbalance_ratio = 0.3
    multiNode_factor = 1    # 多路口的车流量跟单路口相比，可以按比例倍增
    feature_config = {  # middle
        'time_imbalance__space_dynamic_imbalance': {  # note:时间不均 动态空间不均
            'time_dist': [18, 108, 234, 72, 162, 54],
            'space_dist_len': 3,
        },
    }
    # space_imbalance_ratio = 0.2 if 'single' in rou_dir else 0.1
    # feature_config = {  # middle
    #     'time_imbalance__space_dynamic_imbalance': {  # note:时间不均 动态空间不均
    #         'time_dist': [12, 72, 156, 48, 108, 36],
    #         'space_dist_len': 3,
    #     },
    # }

    # space_imbalance_ratio = 0.16 if 'single' in rou_dir else 0.1
    # feature_config = {    # low & high
    #     'time_low_balance__space_balance': {  # note:稀疏车流
    #         'time_dist': [30] * 6,
    #         'space_dist_len': 0,
    #     },
    #     'time_imbalance__space_dynamic_imbalance': {  # note:时间不均 动态空间不均
    #         'time_dist': [30, 180, 390, 120, 270, 90],
    #         'space_dist_len': 3,
    #     },
    # }
    # feature_config = {
    ...
    #     #     'space_dist_len': 0,
    #     # },
    #     # 'time_imbalance__space_static_imbalance': {  # note:时间不均 空间均
    #     #     'time_dist': [30, 180, 390, 120, 270, 90],
    #     #     'space_dist_len': 0,
    #     # },
    #     # 'time_imbalance__space_dynamic_imbalance': {  # note:时间不均 动态空间不均
    ...
    #     #     'space_dist_len': 3,
    #     #
    #     # 'time_imbalance__space_static_imbalance': {  # note:时间不均 动态空间不均
    #     #     'time_dist': [40, 240, 520, 160, 360, 120],
    #     #     'space_dist_len': 0,
    #     # },
    #     # 'time_imbalance__space_dynamic_imbalance': {  # note:时间不均 动态空间不均
    #     #     'time_dist': [40, 240, 520, 160, 360, 120],
    ...
    if 'four' in rou_dir:  # 2*2入口数翻一倍，路口数翻四倍，大概估计把车流量*2(试过2.5)来从single转到2*2
        for _, ft in feature_config.items():
            ft['time_dist'] = [int(_ * multiNode_factor) for _ in ft['time_dist']]
