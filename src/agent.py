...
from algorithm import HATD3Triple, HATD3Double, HATD3, TD3Single, WorkerTD3, ManagerTD3, NaiveEncoder

np.random.seed(3407)  # 设置随机种子


class FullIndependentLightAgent:
    def __init__(self, light_id, config):
        ...
        self.var_p = config['var']
        self.var_t = config['var']
        self.var_g = config['var']
        ...
        self.encoder = NaiveEncoder(2, self.lane_state_dim)    # 例如Encoder用的dim=4，那么实际出来的状态是8，因为是mean+logvar

        self.time_net = TD3Single(config, 'time')
        self.phase_net = TD3Single(config, 'phase')
        # self.phase_net = DQN(config, 'phase')
        if self.lane_agent:
            config['memory_capacity'] = config['memory_capacity'] * 8  # 每次只给一个车道制定目标也会导致存速翻倍
        self.goal_net = TD3Single(config, 'vehicle')    # 因为这里time&phase解耦出去，所以只给goal的memory扩容

        self.save = lambda path, ep: (self.time_net.save(path + 'light_agent_time' + self.holon_name + '_ep_' + str(ep)),
                                      self.phase_net.save(path + 'light_agent_phase' + self.holon_name + '_ep_' + str(ep)),
                                      self.goal_net.save(path + 'light_agent_goal' + self.holon_name + '_ep_' + str(ep)))
        if self.load_model:
            load_ep = str(config['load_model_ep']) if config['load_model_ep'] else '99'
            self.time_net.load('../model/' + config['load_model_name'] + '/light_agent_time' + self.holon_name + '_ep_' + load_ep)
            self.phase_net.load('../model/' + config['load_model_name'] + '/light_agent_phase' + self.holon_name + '_ep_' + load_ep)
            self.goal_net.load('../model/' + config['load_model_name'] + '/light_agent_goal' + self.holon_name + '_ep_' + load_ep)
        ...
        self.goal_state = {light: deque(maxlen=2) for light in self.light_id}
        self.reward_list = []
    ...
    @property
    def pointer(self):
        return self.time_net.pointer

    @property
    def learn_begin(self):
        return self.time_net.learn_begin
        ...
        if self.time_index[light] == 0:
            if self.color[light] == 'y' and self.red != 0:  # 黄灯结束切红灯
                ...
                if not self.use_phase or (not self.train_model and not self.load_model):
                    ...
                    if self.phase_net.__class__.__name__ in ('TD3Single', 'HATD3'):
                        if self.pointer < self.learn_begin and not self.load_model:  # 随机填充
                            a_p = np.random.random(self.phase_net.a_dim) * 2 - 1
                        else:
                            a_p = self.phase_net.choose_phase_action(o_p)
                    ...
                    if self.train_model and len(self.o_p_list[light]) >= 2:
                        self.phase_net.store_transition(self.o_p_list[light][-2], self.a_p_list[light][-2],
                                                        reward, self.o_p_list[light][-1])
                    if self.train_model and self.phase_net.pointer >= self.phase_net.learn_begin:
                        self.var_p = max(0.01, self.var_p * 0.99)
                        self.phase_net.learn()
                next_phase = int(a_p)
                ...
                if not self.use_time or (not self.train_model and not self.load_model):
                    ...
                    if self.pointer < self.learn_begin and not self.load_model:  # 随机填充
                        a_t = np.random.random(self.time_net.a_dim) * 2 - 1
                    else:
                        a_t = self.time_net.choose_time_action(o_t)
                    if self.train_model:    # 加噪声
                        a_t = np.clip(np.random.normal(0, self.var_t, size=a_t.shape) + a_t, -1, 1)
                    self.a_t_list[light].append(a_t)

                    if self.train_model and len(self.o_t_list[light]) >= 2:
                        self.time_net.store_transition(self.o_t_list[light][-2], self.a_t_list[light][-2],
                                                       reward, self.o_t_list[light][-1])
                    if self.train_model and self.time_net.pointer >= self.time_net.learn_begin:
                        self.var_t = max(0.01, self.var_t * 0.99)
                        self.time_net.learn()
                ...
                if self.use_goal:
                    ...
                    if self.lane_agent:
                        ...
                        for i in range(len(env.light_get_lane(light))):
                            ...
                            if self.train_model:    # 加噪声
                                if self.pointer < self.learn_begin and not self.load_model:  # 随机填充
                                    a_g = np.random.random(self.goal_net.a_dim) * 2 - 1
                                else:
                                    a_g = self.goal_net.choose_goal(o_g)
                                a_g = np.clip(np.random.normal(0, self.var_g, size=a_g.shape) + a_g, -1, 1)
                            else:
                                a_g = self.goal_net.choose_goal(o_g)
                            curr_a_g_list.append(a_g)   # [-1,1]

                            g_reward = g_reward_8lane[i]
                            # if len(self.goal_state[light]) >= 1:
                            #     dot = np.dot(enc[-1][i], self.goal_state[light][-1][i])
                            #     norm = np.linalg.norm(enc[-1][i]) * np.linalg.norm(self.goal_state[light][-1][i])
                            #     # 由于当前state和之前goal_state都已经严格clip到[-1,1]，因此余弦相似度不会为负
                            #     exec_res = dot / norm if np.isfinite(norm) and norm != 0 else 0
                            # else:
                            #     exec_res = 1
                            # # g_reward *= exec_res
                            # g_reward *= 1 - exec_res
                            ...
                            if len(self.o_g_list[light]) >= 2:
                                self.goal_net.store_transition(self.o_g_list[light][-2][i], self.a_g_list[light][-2][i],
                                                               g_reward, o_g)

                            curr_r_list.append(2 * reward + g_reward)
                        self.o_g_list[light].append(curr_o_g_list)  # 各lane收集完了再统一更新这个时步的
                        self.a_g_list[light].append(curr_a_g_list)
                        self.goal_state[light].append(np.clip(enc[-1] + curr_a_g_list, 0, 1))
                        ...
                        vehicle_goal = np.array(a_g)

                        g_reward = sum(g_reward_8lane)
                        if len(self.o_g_list[light]) >= 2:
                            self.goal_net.store_transition(self.o_g_list[light][-2], self.a_g_list[light][-2],
                                                           reward, self.o_g_list[light][-1])
                        self.reward_list.append(2 * reward + g_reward)

                    if self.train_model and self.goal_net.pointer >= self.goal_net.learn_begin:
                        self.var_g = max(0.01, self.var_g * 0.99)  # 0.9-40 0.99-400 0.999-4000
                        self.goal_net.learn()
        ...
        if self.time_index[light] == 0:
            if self.color[light] == 'y' and self.red != 0:  # 黄灯结束切红灯
                ...
                if self.lane_agent:
                    ...
                    curr_g = curr_a_g_list[:, 1:] if self.v_in_g else curr_a_g_list
                    self.goal_state[light].append(np.clip(enc[-1] + curr_g, 0, 1))
                    self.reward_list.append(sum(curr_r_list) / len(curr_r_list))
                    # self.reward_list.append(reward)
                    vehicle_goal = np.array(curr_a_g_list)
                else:
        ...
        if self.use_CAV:
            ...
            for cav_id in self.ctrl_cav[light][-1]:
                if cav_id:  # cav is not None
                    o_v = env.get_head_cav_obs(cav_id, next_phase)  # list
    ...
    def _step(self, env, goal: np.ndarray, next_phase, light):
        ...
        if self.use_CAV:
            ...
            if goal is not None:    # 说明上层切相位了，接下来是一对新车道的yrg
                ...
                delta_state = goal[:, 1:] if self.v_in_g else goal
                gs = self.lane_state[light][-1].copy()
                if self.half_goal:
                    gs[:, :delta_state.shape[1]] += delta_state
                else:
                    gs += delta_state
            ...
            for cav_id in self.ctrl_cav[light][-1]:
                ...
                if cav_id:  # cav is not None
                    ...
                    if len(cav_obs) >= self.T:  # 没存满就先不控制
                        ...
                        time_factor = 1 - o_v[6]    # 0830: 随着当前相位的进行，时间因子不断变大，越来越需要考虑上层目标的完成。
                        reward = (1 - self.alpha) * ext_reward + self.alpha * time_factor * int_reward
...
class LoyalCavAgent:
    def __init__(self, light_id, config):
        ...
        self.encoder = NaiveEncoder(2, self.lane_state_dim // 2)    # 例如Encoder用的dim=4，那么实际出来的状态是8，因为是mean+logvar
        self.encoder.load('../model/' + config['encoder_load_path'] + '/encoder_dim_' + str(self.lane_state_dim // 2))
