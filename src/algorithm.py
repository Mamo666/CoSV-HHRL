...
class NaiveEncoder:
    def __init__(self, input_dim, g_dim):
        self.g_dim = g_dim
        if g_dim == 1:
            self.mode = 'ave_speed'
        elif g_dim == 2:
    ...
    def encode(self, xvs: list):
        if self.mode == 'ave_speed':
            return np.full(self.g_dim, np.mean([xv[1] for xv in xvs])).clip(0, 1)
        if len(xvs) == 0:
            xvs = [[.999, .999]]
        if len(xvs) == 1:
            add = [xvs[0][0]-0.1, xvs[0][1]] if xvs[0][0] > 0.5 else [.999, .999]
            xvs.append(add)
        mode = self.mode if len(xvs) >= 4 else 'linear'
        x, v = zip(*xvs)
        interpolator = interp1d(x, v, kind=mode, fill_value='extrapolate', bounds_error=False)
        x_target = np.linspace(0, 1, self.g_dim)
        v_target = interpolator(x_target)
        return v_target.clip(0, 1)

    def decode(self, goal, x):
        if self.mode == 'ave_speed':
            return np.array(goal).reshape(1).clip(0, 1)
        assert len(goal) == self.g_dim, 'Goal length does not match g_dim'
        xs = np.linspace(0, 1, self.g_dim)
        interpolator = interp1d(xs, goal, kind=self.mode, fill_value='extrapolate', bounds_error=False)
        return interpolator(x).clip(0, 1)

    def load(self, path):
        return self.g_dim, path
...
class HATD3(HATD3Double):
    def __init__(self, cfg):
        ...
        self.t2 = cfg['phase']['T']
        self.obs2_dim = self.o2_dim * self.t2
        assert self.t1 == self.t2, '现在基本确定t和p的T没必要不相等'
        self.t = self.t1
...
class WorkerTD3:
    ...
    def learn(self):
        ...
        # E_sa = -torch.norm(f(s飘) - phi(s, a一堆), p=2, dim=-1)
        # log_part = torch.logsumexp(E_sa, dim=0) - torch.log(self.batch_size)
        # J_att = torch.norm(f(next_obs) - phi(s, a一堆), p=2, dim=-1)
        # reward = -torch.norm(f(next_obs) - goal), p=2, dim=-1) + J_att + log_part
        # J_rep = torch.exp(E_sa) - stop_grad(log_part)
        #
        # repr_loss = J_att + J_rep
